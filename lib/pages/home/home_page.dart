...
class _HomePageState extends State<HomePage> {
  ...
  void _findNearbyPlaces(double lat, double lng) async {
    ...
    try {
      final List<dynamic> placesResult = await _apiService.getNearbyPlaces(
        lat,
        lng,
      );
      ...
      if (mounted)
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(SnackBar(content: Text('Error mencari tempat: $e')));
    } finally {
  ...
  void _handleSearch() async {
    final result = await showSearch<Map<String, dynamic>?>(
      context: context,
      delegate: PlaceSearchDelegate(_apiService),
    );
    if (result != null && result.containsKey('place_id')) {
      try {
        ...
        if (mounted)
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('Gagal mendapatkan detail lokasi: $e')),
          );
      }
    }
  }
      ...
      try {
        ...
        if (mounted) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(
                '"$title" berhasil ditambahkan! Cek di tab Jadwal.',
              ),
            ),
          );
        }
        ...
        children: [
          GoogleMap(
            ...
            onTap: (LatLng position) {
              setState(() {
                _selectedMarker =
                    null; // Sembunyikan kartu info saat peta diketuk
                _markers.removeWhere(
                  (marker) => marker.markerId == _customMarkerId,
                );
                final newMarker = Marker(
                  ...
                  onTap: () {
                    setState(() {
                      ...
                      _selectedMarkerLocationAddress =
          ...
          Positioned(
            bottom: _selectedMarker == null
                ? 20.0
                : 170.0, // Posisi naik jika kartu muncul
            right: 15.0,
